    ...
    waitingPage(nullptr)
{
    ...
    ui->closeBtn->setStyleSheet(QString("QToolButton{background-image:url(%1pic2/close3.png);background-repeat: repeat-xy;background-position: center;background-attachment: fixed;background-clip: padding;border-style: flat;}").arg(QCoreApplication::applicationDirPath() +"/"));
...
//              );
    ...
    ui->minimizeCheckBox->setStyleSheet(QString("QCheckBox::indicator{ image:url(%1pic2/checkBox_unchecked.png); }"
                                    "QCheckBox::indicator:checked{ image:url(%2pic2/checkBox_checked.png); }").arg(QCoreApplication::applicationDirPath() +"/"));
...
void SetDialog::on_saveBtn_clicked()
{
	...
	else if (ui->languageComboBox->currentIndex() == 1)
	{
        QString str("Korean");
        DataMgr::getDataMgr()->setLanguage(str);
		DataMgr::getDataMgr()->getSettings()->setValue("/settings/language", "Korean");
	}
...
{
    QDesktopServices::openUrl( QUrl("www.tongtongcoin.com"));
}
