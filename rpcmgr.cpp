#include "rpcmgr.h" //unix is case sensitive Rao
...
void RpcMgr::startBlockChain() {
    ...
    strList2 << "--data-dir" << /*"\""+*/config_file->value("/settings/ttchainPath").toString()/*+"\""*/
	...
	qDebug() << "workpath() + TTCHAIN.exe : " << QCoreApplication::applicationDirPath() + "\\TTCHAIN.exe";
    _formal_chain_process->start(QCoreApplication::applicationDirPath()+"\\TTCHAIN.exe", strList2);
	if (_formal_chain_process->waitForStarted()) {
		initWorkThreadManager();
		qDebug() << "launch TTCHAIN.exe succeeded";
		qDebug() << Misc::read(_formal_chain_process);
		QThread::msleep(1000);
		emit onFormalChainStarted();
	} else {
        qDebug() << "launch TTCHAIN.exe failed : " + _formal_chain_process->errorString();
	}
#else
    //TEST << "--httpdendpoint" << "0.0.0.0:8080"
    QStringList strList2;
    ...
    _formal_chain_process = new QProcess;
    qDebug() << "strList2 : "<< strList2;
    qDebug() << "./TTCHAIN : " << "./TTCHAIN";
    qDebug()<<"Path: "<<  qApp->applicationDirPath();
    QString appPath = qApp->applicationDirPath();
    if(!appPath.endsWith("/"))
        appPath += "/";

    _formal_chain_process->start(appPath +"./TTCHAIN", strList2);
    if (_formal_chain_process->waitForStarted()) {
        initWorkThreadManager();
        qDebug() << "launch TTCHAIN succeeded";
        qDebug() << Misc::read(_formal_chain_process);
        QThread::msleep(1000);
        emit onFormalChainStarted();
    } else {
        qDebug() << "launch TTCHAIN failed : " + _formal_chain_process->errorString();
    }
#endif
}
