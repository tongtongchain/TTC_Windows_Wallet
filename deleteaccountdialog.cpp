...
//    Goopal::getInstance()->appendCurrentDialogVector(this);
	...
	QString titleLabel = "Delete " + accountName + "?";

	ui->titleLabel->setStyleSheet("QLabel {color : red; }");
	ui->label_2->setStyleSheet("QLabel {color : red; }");

	ui->titleLabel->setText(tr(titleLabel.toStdString().c_str()));

	connect(DataMgr::getInstance(), SIGNAL(&DataMgr::onWalletCheckPassphrase), this, SLOT(&DeleteAccountDialog::slot_CheckPassphrase));
...
void DeleteAccountDialog::on_okBtn_clicked()
{
	...
	qDebug() << "CHECK PASSPHRASE: " << result << endl;

	if (result.mid(0, 4) == "true") {

		QString str = "wallet_account_delete "+ accountName + "\n";
		Misc::write(str, RpcMgr::getInstance()->currentProcess());
		QString result = Misc::read(RpcMgr::getInstance()->currentProcess());
		qDebug()<<"INFO RESULT: " << result << endl;

		CommonDialog commonDialog(CommonDialog::OkOnly);

		QString success = accountName + " deleted Successfully. You need to restart the wallet to notice the changes.";
		commonDialog.setText(tr(success.toStdString().c_str()));
		...
		commonDialog.setText(tr("Incorrect Passphrase."));
		bool choice = commonDialog.pop();
		...
		commonDialog.setText(tr("Password length must be at least 8 characters."));
		bool choice = commonDialog.pop();
...
//    else if( result.mid(0,5) == "false")
//    {
//        ui->tipLabel1->setPixmap(QPixmap(QCoreApplication::applicationDirPath() + "/pic2/wrong.png"));
...
void DeleteAccountDialog::slot_CheckPassphrase(QString result)
{
