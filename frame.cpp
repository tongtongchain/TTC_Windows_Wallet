...
#define UPDATE_TOOL_NAME "TTCUpdate.exe"//"AchainUp.exe"


void writeLog(QString log) {
	QFile file("./TTCOIN.log");
...
Frame::Frame() : timer(NULL), timerUpdate(NULL),
	...
	//connect(DataMgr::getDataMgr(), &DataMgr::onWalletSetTransactionScanning, this, &Frame::walletSetTransactionScanning);
	connect(DataMgr::getDataMgr(), &DataMgr::onWalletScanAfterSettingTransactionTrue, this, &Frame::walletScanAfterScanningTrue);
    
    if (!DataMgr::getDataMgr()->getSettings()->contains("/settings/ttchainPath")) {
        ...
        if (!dir.exists() || (!dir2.exists() && !dir3.exists())) { // 如果数据文件被删除了
            ...
            */
			...
			if (!DataMgr::getInstance()->notProduce && DataMgr::getInstance()->lockMinutes < 1)
			{
				DataMgr::getInstance()->lockMinutes = 5;
			}
    ...
    //放在托盘提示信息、托盘图标
	trayIcon->setToolTip(QString("TongTongCoin Wallet ") + TTCOIN_WALLET_VERSION);
    trayIcon ->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/TTCOIN.ico"));
    //点击托盘执行的事件
...
void Frame::alreadyLogin() {
    ...
    connect(functionBar, SIGNAL(signal_showTransferPage()), this, SLOT( showTransferPage()));
	connect(functionBar, SIGNAL(signal_showBackUpPage()), this, SLOT(frame_showBackUpPage()));
	connect(functionBar, SIGNAL(signal_showBulkSendingPage()), this, SLOT(frame_showBulkSendingPage()));
	...
	//
	connect(mainPage, SIGNAL(signal_showMainPage()), this, SLOT(showMainPage()));
	...
	//mod zxj
	if (!(DataMgr::getInstance()->notProduce) && DataMgr::getInstance()->lockMinutes > 0) {
...
void Frame::frame_showBackUpPage() {
	//qDebug() << "Inside here frame" << endl;
	closeCurrentPage();
	//getAccountInfo();
	backuppage = new BackUpDialog(centralWidget);
	//connect(backuppage, SIGNAL(showShadowWidget()), this, SLOT(shadowWidgetShow()));
	//connect(backuppage, SIGNAL(hideShadowWidget()), this, SLOT(shadowWidgetHide()));
	//connect(backuppage, SIGNAL(gotoTransferPage(QString)), this, SLOT(showTransferPageWithAddress(QString)));
	backuppage->setAttribute(Qt::WA_DeleteOnClose);
	backuppage->show();
	currentPageNum = 5;
	functionBar->choosePage(5);
}

void Frame::frame_showBulkSendingPage() {
	//qDebug() << "Inside here frame" << endl;
	closeCurrentPage();
	//getAccountInfo();
	bulkSending = new BulkSending("", centralWidget);

	bulkSending->setAttribute(Qt::WA_DeleteOnClose);
	bulkSending->show();
	currentPageNum = 4;
...
void Frame::setLanguage(QString language) {
    ...
    if (language == "Simplified Chinese") {
        menuTranslator.load(QCoreApplication::applicationDirPath() +"/" + "language/qt_zh_CN.qm");
        translatorForTextBrowser.load(QCoreApplication::applicationDirPath() +"/" + "language/widgets_cn.qm");
        translator.load(QCoreApplication::applicationDirPath() +"/" + "language/ttcwallet_simplified_Chinese.qm");
        QApplication::installTranslator(&menuTranslator);
        QApplication::installTranslator(&translatorForTextBrowser);
    } 
	else if(language == "Korean") {
		menuTranslator.load(QCoreApplication::applicationDirPath() +"/" + "language/qtbase_ko.qm");
		translatorForTextBrowser.load(QCoreApplication::applicationDirPath() +"/" + "language/widgets_ko.qm");
		translator.load(QCoreApplication::applicationDirPath() +"/" + "language/ttcwallet_ko.qm");
		QApplication::installTranslator(&menuTranslator);
		QApplication::installTranslator(&translatorForTextBrowser);
	}
	else if (language == "English") {
        translator.load(QCoreApplication::applicationDirPath() +"/" + "language/ttcwallet_English.qm");
    ...
    if( titleBar != NULL) {     // 已经登录
        ...
        switch (currentPageNum) {
            ...
            case 5:
				...
				frame_showBackUpPage();
                break;
                
            case 4:
				frame_showBulkSendingPage();
                break;
...
void Frame::walletListAccounts(QString result) {
    ...
    foreach (QString str, accountInfoList) {
		...
		//add zxj
		mutexForBalanceinfomap.lockForRead();
		info.balanceinfos = (*(DataMgr::getDataMgr()->getAccountBalanceInfo()))[accountName];
		mutexForBalanceinfomap.unlock();
        //todo
    ...
    if( DataMgr::getDataMgr()->getCurrentAccount().isEmpty()) {
        // 默认是 map中第一个账户
		QString straccountName = accountList.at(0);
		DataMgr::getDataMgr()->setCurrentAccount(straccountName);
        
    } else if( !accountList.contains(DataMgr::getDataMgr()->getCurrentAccount())) {
...
void Frame::walletAccountBalance(QString result) {
    ...
    //foreach(QString name, accountList) {
		...
		//test log
		//{
		//	QString log("return balance result:");
		//	log += Jsonstr;
		...
		if (!doucment.isNull() && (jsonError.error == QJsonParseError::NoError)) {  // 解析未发生错误
			...
			//mod zxj 
			mutexForBalanceinfomap.lockForWrite();
			QMap<QString, QMap<int, __int64>>::iterator it1;
			for (it1 = DataMgr::getDataMgr()->getAccountBalanceInfo()->begin(); it1 != DataMgr::getDataMgr()->getAccountBalanceInfo()->end(); ++it1)
			{
				if (it1.value().size() > functionBar->getAssetNum())
				{
					functionBar->refreshAssetCombobox();
				}
				QMap<int, __int64>::iterator it2;
									...
									if (value1.isString()){
										...
										for (int k = 0; k < nSize; ++k) {
											...
											if (value2.isArray()) {
												...
												mutexForBalanceinfomap.lockForWrite();
												((*DataMgr::getDataMgr()->getAccountBalanceInfo())[accountname])[id] = balance;
												mutexForBalanceinfomap.unlock();
											}
										}
									}
        ...
        if (p != -1) { // 如果balance中存在
            ...
            info.balance = QString::number(amount, 'f', 5) + " TTCOIN";
            (*DataMgr::getDataMgr()->getAccountInfo())[name] = info;
            
        } else {
            CommonAccountInfo info = DataMgr::getDataMgr()->getAccountInfo()->value(name);
            info.balance = "0.00000 TTCOIN";
            (*DataMgr::getDataMgr()->getAccountInfo())[name] = info;
        }*/
    //}
}
...
void Frame::init() {
	...
	qDebug() << "In INIT of FRAME........." << endl;

	DataMgr::getInstance()->walletSetTransactionScanning(true);
...
void Frame::walletSetTransactionScanning(QString result)
{
	qDebug() << "RESULT SCANING: " << result << endl;
	qDebug() << "MID: " << result.mid(0, 13) << endl;
	if (result.mid(0, 13) == "\"result\":true") {
		// record_id 被改为了 entry_id
		qDebug() << "INSIDE SIGNAL SLOT: " << endl;
		DataMgr::getInstance()->walletScanAfterScanningTrue();
	}
...
void Frame::walletScanAfterScanningTrue(QString result)
{
	qDebug() << "RESULT SCAN HERE: " << result << endl;
}
