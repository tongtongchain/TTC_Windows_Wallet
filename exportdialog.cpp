    ...
    accountName(name)
{
	...
	foreach(str, environment) {
		if (str.startsWith("APPDATA=")) {
			_app_data_path = str.mid(8) + "\\TTCOIN\\keys";
			qDebug() << "appDataPath (Key):" << _app_data_path;
			break;
		}
	}

	if (!QDir(_app_data_path).exists())
		QDir().mkdir(_app_data_path);

	exportPath = _app_data_path;

	qDebug() << "KeySaving File: " << exportPath << endl;
#endif
    ...
    QStringList environment = QProcess::systemEnvironment();
        ...
        foreach(str, environment) {
                if (str.startsWith("HOME=")) {
                    exportPath = str.mid(5) + "/TTCOIN/keys";
                    qDebug() << "TTCOIN keys Export Path:" << exportPath;
                    break;
                }
            }
        //exportPath = QDir::homePath() + "/TTCOINWallet/keys";
        ui->pathLineEdit->setText(exportPath + "/" + accountName + ".gkey");
#endif // WIN32 //zxlrun
