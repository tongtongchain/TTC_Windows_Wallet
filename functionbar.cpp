...
ui(new Ui::FunctionBar),wait_for_update(0)
{
	...
	setPalette(palette);
	ui->activeLabel->setPixmap(QPixmap(QCoreApplication::applicationDirPath() + "/pic2/active.png"));
	ui->activeLabel->setScaledContents(true);
	ui->logoLabel->setPixmap(QPixmap(QCoreApplication::applicationDirPath() + "/pic2/logo.png"));
	...
	//connect(ui->button_backup, SIGNAL(clicked()), this, SLOT(on_backupBtn_clicked()));

	ui->button_backup->hide();

    //增加下拉框图片
#ifdef WIN32
    QPixmap icon1(QCoreApplication::applicationDirPath() + "/pic2/arrowdown.png");
    ui->assertComboBox->setIconSize(QSize(30, 20));
    ui->assertComboBox->setWindowIcon(icon1);


#endif // WIN32//zxlrun
    ui->versionLabel_2->setText("TTCOIN Wallet\r\n" "Ver." TTCOIN_WALLET_VERSION);
    ...
    ui->assertComboBox->setStyleSheet(
                ...
                "QComboBox::drop-down {"
                        ...
                        "height:30px;"
                "}"
                "QComboBox::down-arrow { image: url(%1pic2/assetComboxArrow.png);}").arg(QCoreApplication::applicationDirPath() +"/")
              );
/*    ui->assertComboBox->setStyleSheet(
...
void FunctionBar::setFontPixelSize()
{
	...
	ui->button_backup->setFont(font);
	ui->button_bulk_sending->setFont(font);
...
void FunctionBar::setAssetInfo(QString result)
{
						...
						if (value1.isObject()) {
							...
							if (val.toInt() == 0) {
								...
								if (object.contains("symbol")) {
									...
									//if (assetinfo.symbol == "TTCOIN" || assetinfo.symbol == "TTM" || assetinfo.symbol == "TTMI")
									//	ui->assertComboBox->addItem(assetinfo.symbol);

									/*if (assetinfo.symbol == "TTCOIN")
										ui->assertComboBox->addItem(assetinfo.symbol);

									else if(assetinfo.symbol == "TTM")
										ui->assertComboBox->addItem(assetinfo.symbol);

									else if (assetinfo.symbol == "TTMI")
										ui->assertComboBox->addItem(assetinfo.symbol);*/

									//qDebug() << "Symbol: " << val.toString();
								}
    ...
    /*ui->assertComboBox->clear();
    ui->assertComboBox->addItem("TTCOIN");
    foreach(ContractFromServer tokenCon, DataMgr::getInstance()->getContractConfig())
    {
        ui->assertComboBox->addItem(tokenCon.name);
    }*/
...
void FunctionBar::on_button_backup_clicked()
{

	CommonDialog commonDialog(CommonDialog::OkOnly);

	commonDialog.setText(tr("Under Development!!"));
	...
	//emit signal_showBackUpPage();

	
}

void FunctionBar::on_button_bulk_sending_clicked()
{
	choosePage(4);
	emit signal_showBulkSendingPage();
}
...
//    ui->activeLabel->move(0, 17 + 94 * ( pageIndex - 1));
    ...
    switch (pageIndex) {
    case 1:
        ui->surveyBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/surveyBtn.png"));
        ...
        ui->accountBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/accountBtn_unselected.png"));
        ...
        ui->delegateBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/delegateBtn_unselected.png"));
		...
		ui->button_backup->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/backup_unselected.png"));
		ui->button_backup->setIconSize(QSize(26, 26));
		ui->button_backup->setStyleSheet("background:transparent;color:grey");
		ui->button_backup->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);

		ui->button_bulk_sending->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/bulkSending_unselected.png"));
		ui->button_bulk_sending->setIconSize(QSize(26, 26));
		ui->button_bulk_sending->setStyleSheet("background:transparent;color:grey");
		ui->button_bulk_sending->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);

        ui->transferBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/transferBtn_unselected.png"));
        ...
        ui->contactBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/contactBtn_unselected.png"));
    ...
    case 2:
        ui->surveyBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/surveyBtn_unselected.png"));
        ui->surveyBtn->setIconSize(QSize(29,28));
        ui->surveyBtn->setStyleSheet("background:transparent;color:grey");
        ui->surveyBtn->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
        ui->accountBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/accountBtn.png"));
        ui->accountBtn->setIconSize(QSize(24,17));
    ...
    case 3:
        ...
        ui->accountBtn->setStyleSheet("background:transparent;color:grey");
        ui->accountBtn->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
        ui->transferBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/transferBtn.png"));
        ui->transferBtn->setIconSize(QSize(23,18));
    ...
    case 5:
        ...
        ui->delegateBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/delegateBtn.png"));
        ui->delegateBtn->setIconSize(QSize(26,26));
        ui->delegateBtn->setStyleSheet("background:transparent;color:white");
        ui->delegateBtn->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);

		ui->button_backup->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/backup_selected.png")); 
		//qDebug() << "I am here inside choose";
		ui->button_backup->setIconSize(QSize(26, 26));
		ui->button_backup->setStyleSheet("background:transparent;color:grey");
		ui->button_backup->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
    ...
    case 4:
		...
		ui->button_bulk_sending->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/bulkSending_selected.png"));
		ui->button_bulk_sending->setIconSize(QSize(26, 26));
		ui->button_bulk_sending->setStyleSheet("background:transparent;color:grey");
		ui->button_bulk_sending->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
        ...
        //ui->contactBtn->setIcon(QIcon(QCoreApplication::applicationDirPath() + "/pic2/contactBtn.png"));
        //ui->contactBtn->setIconSize(QSize(23,22));
...
int FunctionBar::getAssetNum()
{
	return ui->assertComboBox->count();
}

void FunctionBar::refreshAssetCombobox()
{
	DataMgr::getDataMgr()->blockChainListAssets();
}
