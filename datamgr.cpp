...
int DataMgr::getMainAssetPrecision()
{
	return _asset_info_map[0].precision;
}


QString DataMgr::getAccountAddr(QString account)
{
	QString address = DataMgr::getInstance()->getAccountInfo()->value(account).address;
	return address;
}
...
void DataMgr::walletScanAfterScanningTrue(){
	_wallet.walletScanAfterSettingTransactionTrue();
}
...
double DataMgr::getCurBalanceByAssettypeAndAccountname(QString assettype, QString accountname)
{
	...
	if (findid)
	{
		precision = _asset_info_map[asset_id].precision;
	}
	mutexForBalanceinfomap.lockForRead();
	//curbalance = longNumToString(_account_info_map[accountname].balanceinfos[asset_id], precision);
	curbalance = longNumToString((_account_balance_map[accountname])[asset_id], precision);
...
QString DataMgr::walletCurBalanceByAccountname(QString accountname)
{
	...
	//test log
	//{
	//	QString log("walletCurBalanceByAccountname:");

	//	log += "balances:";
	//	QMap<QString, QMap<int, __int64>>::iterator iter;
	//	for (iter = _account_balance_map.begin(); iter != _account_balance_map.end(); iter++)//_account_info_map
	//	{
	//		log += QString::number((*iter)[asset_id]);
	//		log += " | ";
	//	}
	...
	//strBalance = longNumToString(_account_info_map[accountname].balanceinfos[asset_id], precision);
	strBalance = longNumToString((_account_balance_map[accountname])[asset_id], precision);
	mutexForBalanceinfomap.unlock();
	return strBalance;
}
...
{
	double sscbalance = getCurBalanceByAssettypeAndAccountname("TTCOIN", accountname);
	
	double new_min_transaction_fee = 0.1;

	if (assetType == "TTCOIN")
		new_min_transaction_fee = 0.1;
	else
		new_min_transaction_fee = 0.1;
	/*if ((sscbalance + 0.000001) < new_min_transaction_fee)
		return false;*/
	//mod 
	/*if (assetType == "TTCOIN")
	...
	/*if (assetType == "TTCOIN")
	{
		double transfee = amount*0.003000;
		if (transfee < 0.00001)
		{
			transfee = 0.00001;
		}
	...
	double transfee = amount * 0.005;

	if (transfee < new_min_transaction_fee)
		{
			transfee = new_min_transaction_fee;
		}
...
void DataMgr::tokenQueryTransferlog(QString conAddr, QString call_name)
{
    ...
    QString temp("");
    QString act("TTCOIN");
//zxltab	DataMgr::getInstance()->callContractTesting(conAddr, call_name, QString("query_transfer_log"), QString(""), QString("TTCOIN"));
...
void DataMgr::tokenQueryBalance(QString conName, QString call_name)
{
    ...
    QString temp("");
    QString act("TTCOIN");
    //zxltabDataMgr::getInstance()->callContractTesting(call_addr, call_name, QString("query_balance"), QString(""), QString("TTCOIN"));
...
void DataMgr::tokenTransferTo(QString call_name, QString assetType, QString to_address, double amount, double fee, QString remark)
{
    ...
    QString temp("");
    QString act("TTCOIN");
    //zxltabDataMgr::getInstance()->callContract(call_addr, call_name, QString("transfer_to"), param, QString("TTCOIN"), fee);
...
void DataMgr::parseTrxResultJson(QString trx_json, QVector<TrxResult>& trxes) {
	...
	for (int i = 0; i < count; i++) {
		if (json_array.at(i).isObject()) {
			...
			result.abolished= trx_object.value(QString("is_abolished")).toBool();
...
void DataMgr::onUpdateRpcResult(QString id, QString data) {
	...
	if (id.startsWith("id_scan")) {
		emit onWalletScanAfterSettingTransactionTrue(data);
	}
