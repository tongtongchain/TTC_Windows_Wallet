    ...
    ui(new Ui::LockPage)
{
    ...
    ui->logoLabel->setPixmap(QPixmap(QCoreApplication::applicationDirPath() + "/pic2/TTCWallet.png"));

	QString language = DataMgr::getInstance()->getSettings()->value("/settings/language").toString();
    if( language.isEmpty() || language == "English")
    {
        ui->lockedLabel->setPixmap(QPixmap(QCoreApplication::applicationDirPath() + "/pic2/locked_En.png"));
        ui->lockedLabel->setGeometry(454,310,72,13);
    }
	else if (language == "Korean")
	{
		ui->lockedLabel->setPixmap(QPixmap(QCoreApplication::applicationDirPath() + "/pic2/locked_Ko.png"));
		ui->versionLabel->setGeometry(100, 12, 81, 14);
	}
    else if( language == "Simplified Chinese")
    {
        ui->lockedLabel->setPixmap(QPixmap(QCoreApplication::applicationDirPath() + "/pic2/locked.png"));
    }
    ...
    ui->unlockBtn->setStyleSheet(QString("QToolButton{background-image:url(%1pic2/arrow.png);background-repeat: repeat-xy;background-position: center;background-attachment: fixed;background-clip: padding;border-style: flat;}"
                                 "QToolButton:hover{background-image:url(%2pic2/arrow_hover.png);background-repeat: repeat-xy;background-position: center;background-attachment: fixed;background-clip: padding;border-style: flat;}").arg(QCoreApplication::applicationDirPath() +"/").arg(QCoreApplication::applicationDirPath() +"/"));
